...
export const LoadableCmsProvider: React.FC<React.PropsWithChildren<{}>> = ({
  ...
  const findUrlKey = hooks.useApiCallBack(
    async (api, args: { currentKey: string }) =>
      await api.cms.filterMechanism(args)
  );
  ...
  return (
    <LoadableContext.Provider value={{}}>{children}</LoadableContext.Provider>
  );
};
